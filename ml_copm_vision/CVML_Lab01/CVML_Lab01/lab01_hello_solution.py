...
def arrays(args : argparse.Namespace):
    ...
    ones = np.ones((5, 10), dtype=int)
    print("Ones:\n{}".format(ones))
    # TODO: Create an object array of 'None's with shape (10,).
    nones = np.asarray([None for _ in range(10)], dtype=object)
    ...
    linspace_arr = np.linspace(0, 1.0, 9, endpoint=True)
...
def functions(args : argparse.Namespace):
    ...
    result = x_data ** 3 / 2 - 5 * x_data * y_data ** 2 + 3 * x_data + 3
    ...
    res_vec = from_orig @ rot_mat @ to_orig @ v_vec
